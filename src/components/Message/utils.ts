 ...
 * @returns { MessageType } result 返回一个带有then()属性的可执行函数；可执行函数为 openFn() 高阶函数的返回值
 *  - 一般 openFn() 高阶函数的返回值 VoidFunction 中包含 resolve() ，作为 wrapPromiseFn 的出口使用
...
export const wrapPromiseFn = (openFn: (resolve: VoidFunction) => VoidFunction): MessageType => {
    /**
     * openFn 高阶函数的返回值，是() => { resolve() }的扩展形式
